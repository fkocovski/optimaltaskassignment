...
with tf.name_scope("weights"):
    weights = {
        ...
        'h4': tf.Variable(var_multiplicator * tf.random_normal([n_hidden_2, n_hidden_3],seed=SEED+8), name="h4"),
...
with tf.name_scope("biases"):
    biases = {
        ...
        'b4': tf.Variable(var_multiplicator * tf.random_normal([n_hidden_3],seed=SEED+9), name="b4"),
...
with tf.name_scope("neural_network"):
    ...
    pred = [tf.add(tf.matmul(layer_4, weights['out'][b]), biases['out'][b]) for b in
            range(batch_input)]
...
with tf.name_scope("optimizer"):
    ...
    gradients = [optimizer.compute_gradients(cost[b], [weights["h1"],weights["h2"],weights["h3"],weights["h4"],
                                                       weights["out"][b],
                                                       biases["b1"],biases["b2"],biases["b3"],biases["b4"], biases["out"][b]]) for b in range(batch_input)]
...
with tf.name_scope("summaries"):
    ...
    summary_h4 = tf.summary.histogram("h4", weights["h4"])
    ...
    summary_b4 = tf.summary.histogram("b4", biases["b4"])
