...
from policies.reinforcement_learning.others.bi_one_mc_tf_1l import BI_ONE_MC_TF_1L
...
policy_name = "{}_BI_ONE_MC_TF_1L_NU{}_GI{}_SIM{}".format(batch_input, NUMBER_OF_USERS, GENERATION_INTERVAL,
                                                              SIM_TIME)

with tf.name_scope("weights"):
    weights = {
        'h1': tf.Variable(var_multiplicator*tf.random_normal([n_input, n_hidden_1]), name="h1"),
        'out': [tf.Variable(var_multiplicator*tf.random_normal([n_hidden_1, n_out]), name="out") for _ in
                range(batch_input)]
    }
...
with tf.name_scope("neural_network"):
    ...
    pred = [tf.add(tf.matmul(layer_1, weights['out'][b]), biases['out'][b]) for b in
                 range(batch_input)]
...
with tf.Session() as sess:
    ...
    for i in range(epochs):
        ...
        policy_train = BI_ONE_MC_TF_1L(env, NUMBER_OF_USERS, WORKER_VARIABILITY, None, gamma, False, BATCH_SIZE, sess,
    ...
    policy = BI_ONE_MC_TF_1L(env, NUMBER_OF_USERS, WORKER_VARIABILITY, file_policy, gamma, True, BATCH_SIZE, sess,
