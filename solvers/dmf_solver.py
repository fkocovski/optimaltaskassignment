...
def dmf(a, p, w, batch_queue_length, number_of_users):
    """
DMF solver.
    ...
    # zik
    z = [[model.addVar(vtype=GRB.CONTINUOUS, name="z{}{}".format(i, k)) for k in range(batch_queue_length)] for i
         in range(number_of_users)]
    ...
    for i in range(number_of_users):
        for k in range(batch_queue_length):
            if k > 0:
                model.addConstr(
                    z[i][k - 1] + quicksum(p[i][j] * x[i][j][k] for j in range(batch_queue_length)) <= z[i][k])
            else:
                model.addConstr(
                    a[i] + quicksum(p[i][j] * x[i][j][k] for j in range(batch_queue_length)) <= z[i][k])
            model.addConstr(z[i][k] + quicksum(w[j] * x[i][j][k] for j in range(batch_queue_length)) <= z_max)
