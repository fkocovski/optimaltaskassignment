...
class K_BATCH_TD_VFA(Policy):
    def __init__(self, env, number_of_users, worker_variability, file_policy, batch_size, theta,
        ...
        self.name = "{}_BATCH_TD_VFA".format(self.batch_size)
    ...
    def evaluate(self, k_batch_job):
        a, p, states = self.state_space(k_batch_job)
    ...
    def update_theta_td(self):
        for i, (states, action) in enumerate(self.history):
            try:
                self.theta += self.alpha * (-(
                    self.rewards[i + 1] + self.gamma * self.action_value_approximator(self.history[i + 1][0],
                                                                                      self.history[i + 1][
                                                                                          1])) - self.action_value_approximator(
                    states, action)) * self.gradient(states, action)
            except IndexError:
                return
