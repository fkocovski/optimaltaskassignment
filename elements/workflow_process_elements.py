...
class StartEvent(Node):
    def __init__(self, env, generation_interval, actions, weights,seed=1,accelerate=False,starting_generation=None,sim_time=None):
        """
...
Generates infinitely many tokens (implicit objects) following an exponential rate.
    ...
    def sigmoid(self,t):
        alpha = 0.01
        sig = (self.starting_generation - self.generation_interval) / (1 + np.exp(alpha*(t - self.sim_time/2))) + self.generation_interval
        return sig
