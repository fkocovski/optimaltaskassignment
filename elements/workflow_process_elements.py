...
class StartEvent(Node):
    def __init__(self, env, generation_interval, actions, weights,master_state,accelerate,starting_generation,sim_time,sigmoid_param):
        """
Initializes a start event object.
        ...
        self.child = None
        # self.RANDOM_STATE_ARRIVAL = np.random.RandomState(seed)
        # self.ACTIONS_RANDOM_STATE = np.random.RandomState(seed)
...
Generates infinitely many tokens (implicit objects) following an exponential rate.
    ...
    def sigmoid(self,t):
        sig = (self.starting_generation - self.generation_interval) / (1 + np.exp(self.sigmoid_param*(t - self.sim_time/2))) + self.generation_interval
        self.t.append(t)
        self.g.append(sig)
        return sig
